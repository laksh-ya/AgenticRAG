...
We evaluated KARMA on five major U.S.\ equities: AAPL, MSFT, GOOGL, NVDA, and AMZN. The system used GPT-4o (deep reasoning) and GPT-4o-mini (analysts), with ChromaDB and OpenAI \texttt{text-embedding-3-small} embeddings.

The evaluation followed a \textbf{5-day rolling protocol} over two rounds (Round~1: March~3--9, 2026; Round~2: March~9--13, 2026). Three tickers (AAPL, GOOGL, AMZN) were analyzed daily using the \texttt{balanced\_tech} portfolio preset (10~AAPL, 8~MSFT, 12~GOOGL, 8~NVDA, 10~AMZN). For each daily decision, the T+1 closing price was fetched the following trading day to compute actual returns and evaluate correctness.
...
\subsection{5-Day Rolling Evaluation Results}

Table~\ref{tab:round1} presents Round~1 results and Table~\ref{tab:round2} presents Round~2:

\begin{table}[htbp]
\caption{Round 1 Evaluation Results (March 3--9, 2026)}
\label{tab:round1}
\centering
\footnotesize
\begin{tabular}{@{}clccccc@{}}
\toprule
\textbf{Day} & \textbf{Ticker} & \textbf{Entry} & \textbf{Decision} & \textbf{T+1} & \textbf{Return} & \textbf{Correct} \\
\midrule
1 & AAPL & \$263.74 & HOLD & \$257.68 & $-$2.30\% & \checkmark \\
1 & GOOGL & \$298.63 & HOLD & \$300.24 & +0.54\% & \checkmark \\
1 & AMZN & \$203.77 & HOLD & \$210.18 & +3.15\% & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[htbp]
\caption{Round 2 Evaluation Results (March 9--12, 2026)}
\label{tab:round2}
\centering
\footnotesize
\begin{tabular}{@{}clccccc@{}}
...
\textbf{Day} & \textbf{Ticker} & \textbf{Entry} & \textbf{Decision} & \textbf{T+1} & \textbf{Return} & \textbf{Correct} \\
\midrule
1 & AAPL & \$257.68 & HOLD & \$261.55 & +1.50\% & \checkmark \\
1 & GOOGL & \$300.24 & HOLD & \$308.30 & +2.68\% & \checkmark \\
1 & AMZN & \$210.18 & HOLD & \$215.19 & +2.38\% & \checkmark \\
2 & AAPL & \$261.55 & HOLD & \$260.62 & $-$0.36\% & \checkmark \\
2 & GOOGL & \$308.30 & HOLD & \$308.80 & +0.16\% & \checkmark \\
2 & AMZN & \$215.19 & HOLD & \$210.04 & $-$2.39\% & \checkmark \\
4 & AAPL & \$255.69 & HOLD & --- & --- & --- \\
4 & GOOGL & \$303.75 & HOLD & --- & --- & --- \\
4 & AMZN & \$210.04 & HOLD & --- & --- & --- \\
\bottomrule
...
\begin{table}[htbp]
\caption{Aggregate Evaluation Metrics}
\label{tab:aggregate}
\centering
\begin{tabular}{@{}lr@{}}
\toprule
...
Decisions checked (T+1) & 9 \\
Win rate & \textbf{100\%} (9/9) \\
Avg.\ T+1 return (HOLD) & +0.60\% \\
Avg.\ confidence & 0.79 \\
HOLD / BUY / SELL ratio & 91\% / 9\% / 0\% \\
\bottomrule
\end{tabular}
\end{table}

The system exhibited a strong conservative bias, predominantly issuing HOLD decisions. This stemmed from the portfolio's tech-sector concentration---GOOGL exceeded the 25\% threshold, triggering warnings that propagated through all prompts. The HOLD decisions proved correct across all checked outcomes, as T+1 returns ranged from $-$2.39\% to +3.15\%.
...
\textbf{Case 2---GOOGL 2026-02-17 (Sole BUY):} The Research Manager overrode bearish technicals (RSI 32, 5\% decline) for strong fundamentals (\$126.84B cash, 18\% YoY growth, ROE 35.71\%), citing oversold conditions as a buying opportunity.

\textbf{Case 3---Round 2 Day 1 (KB-Enriched):} Analysts explicitly referenced Round~1's stored context: ``The portfolio is heavily weighted towards technology stocks, with AAPL and GOOGL together comprising nearly half of the total value.'' This portfolio-aware reasoning emerged from KB context interaction.
...
A notable finding is the system's 91\% HOLD rate. Root-cause analysis reveals this originates from portfolio composition rather than an inherent model bias:
...
The sole BUY (GOOGL, 2026-02-17) occurred when oversold technicals (RSI~32) presented a compelling enough signal for the Research Manager to override concentration warnings. This suggests the system correctly balances portfolio-level risk against opportunity, but the threshold may need tuning for less conservative behavior.
...
\item \textbf{Conservative Bias}: The 91\% HOLD rate, while correct in outcomes, limits evaluation of BUY/SELL accuracy. Diversified portfolios or varied concentration thresholds would produce richer signal distributions.
\item \textbf{Evaluation Scale}: Two 5-day rounds (12 decisions) demonstrate the mechanism but are insufficient for statistical significance. Extended multi-month evaluation is needed to validate long-term learning effects.
\end{itemize}
...
Our evaluation over two 5-day rolling rounds (12 decisions, 9 with T+1 outcomes) demonstrated 100\% directional accuracy, with HOLD decisions correctly anticipating mixed-volatility conditions across AAPL, GOOGL, and AMZN. The knowledge base grew from 0 to 39 entries across sessions, with Round~2 analyses explicitly referencing Round~1 outcomes---confirming the closed-loop learning mechanism. The system's conservative bias (91\% HOLD) emerged from portfolio-aware risk management rather than model limitations, as evidenced by the single BUY decision correctly identifying an oversold opportunity in GOOGL.
