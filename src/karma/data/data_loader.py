...
Architecture:
    1. Check JSON cache (<data_dir>/<subdir>/<TICKER>.json) — instant, no network
...
def _get_sources():
    global _sources
    if _sources is None:
        from karma.data import sources
        _sources = sources
    return _sources
...
# Default fallback chains — can be overridden via karma.config.CONFIG
...
class DataLoader:
    ...
    def load(
        ...
        return (
            ...
            f"Place a JSON file at {self.data_dir}/{subdir}/{ticker}.json or check API keys."
        )
