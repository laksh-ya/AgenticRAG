% AgenticRAG - IEEE Conference Paper
...
\title{AgenticRAG: A Multi-Agent Framework with Active Knowledge Base Learning for Autonomous Financial Investment Decisions}
...
\end{tikzpicture}
\caption{AgenticRAG system architecture. The 12-node LangGraph pipeline features bidirectional RAG integration via the KB Agent. Dashed arrow shows the feedback loop: lessons learned from outcomes inform future pre-queries.}
...
We evaluated AgenticRAG on five major U.S.\ equities: AAPL, MSFT, GOOGL, NVDA, and AMZN. The system used GPT-4o (deep reasoning) and GPT-4o-mini (analysts), with ChromaDB and OpenAI \texttt{text-embedding-3-small} embeddings.
...
+5 outcomes & 10 & 5 & 5 & 20 \\
+10 runs \& outcomes & 20 & 10 & 10 & 40 \\
\bottomrule
\end{tabular}
\end{table}
...
The AAPL analysis (2026-02-07) demonstrates context-aware reasoning: despite bullish momentum (+7.37\% monthly return) and positive news (Tim Cook announcement, analyst targets at \$300), the Trader correctly weighed concerning fundamentals (D/E ratio 102.63, current ratio below 1.0) and issued a HOLD at 0.75 confidence. This balanced synthesis is enabled by the Research Manager's structured weighing of opposing bull/bear arguments.

\subsection{Outcome Learning Demonstration}

To illustrate the feedback loop:
\begin{enumerate}
\item \textbf{Session 1}: BUY issued for NVDA at 0.80 confidence.
\item \textbf{Outcome}: After 5 days, NVDA returns $-$4.2\%.
\item \textbf{Reflection}: Lesson generated: ``FAILED BUY ($-$4.20\%). Missed overbought RSI and declining volume.''
\item \textbf{Session 2}: KB Pre-Query retrieves this lesson; analysts reference the prior failure, producing a more cautious assessment.
\end{enumerate}
...
AgenticRAG makes three primary contributions:
...
Within the taxonomy of Singh et al.~\cite{singh2025agenticrag}, AgenticRAG implements elements from multiple categories: Multi-Agent RAG (specialized agent collaboration), Corrective RAG (outcome learning as delayed correction), and Adaptive RAG (dynamic context adjustment). We term this combination \textbf{Reflective Agentic RAG}---an architecture where the system's outputs become future inputs through structured reflection.

\subsection{Limitations and Future Work}

\begin{itemize}
\item \textbf{Outcome Latency}: Lessons require actual returns (T+5 to T+30 days). Future integration with paper trading APIs could automate this.
...
\item \textbf{Quantitative Evaluation}: Rigorous benchmarking against S\&P~500 buy-and-hold over extended periods would strengthen empirical claims.
\end{itemize}
...
Our analysis demonstrates that the system produces contextually informed, fully explainable investment recommendations. The outcome learning mechanism enables the system to reference past successes and failures in future analyses---a capability absent from both passive RAG and existing multi-agent trading frameworks. AgenticRAG represents a step toward truly adaptive AI systems in finance: systems that not only analyze markets but learn from their own experience.
