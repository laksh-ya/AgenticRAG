# AgenticRAG: A Multi-Agent Framework with Active Knowledge Base Learning for Autonomous Financial Investment Decisions
...
Large Language Models (LLMs) have demonstrated considerable promise in financial text analysis, yet their deployment in autonomous investment decision-making remains constrained by three fundamental limitations: the absence of temporal memory across trading sessions, reliance on static knowledge that cannot adapt to evolving market conditions, and opaque reasoning that lacks explainability. While Retrieval-Augmented Generation (RAG) has emerged as a solution for grounding LLM outputs in external knowledge, existing implementations in finance remain predominantly *passive* — retrieving static documents without learning from their own performance. This paper introduces **AgenticRAG**, a novel multi-agent framework featuring an *Active Knowledge Base (KB) Agent* that transforms RAG from a read-only retrieval mechanism into a read-write learning system. The KB Agent operates across three phases of the decision lifecycle: **pre-query** (retrieving historical context and lessons learned before analysis), **post-learn** (storing structured decision records after trading), and **outcome reflection** (generating lessons from realized returns to improve future decisions). Built on LangGraph with a 12-node agent pipeline and ChromaDB with three specialized vector stores, AgenticRAG orchestrates four domain analysts, bull/bear researchers, a risk manager, and a trader agent — all grounded by an evolving institutional memory. Experimental evaluation on a portfolio of five major equities demonstrates that the system produces contextually informed, explainable investment recommendations that improve over successive trading sessions through its self-reinforcing feedback loop.
...
**Figure 1.** AgenticRAG system architecture showing the 12-node LangGraph pipeline with bidirectional RAG integration.
...
A critical advantage of AgenticRAG is observable knowledge accumulation. Table 2 shows how the three ChromaDB stores grow over successive trading sessions:

**Table 2.** Knowledge base growth across trading sessions.

| Session | market_insights | trade_history | lessons_learned | Total |
|---------|----------------|---------------|-----------------|-------|
| Initial | 0 | 0 | 0 | 0 |
| After 1 run | 2 | 1 | 0 | 3 |
| After 5 runs | 10 | 5 | 0 | 15 |
| After 5 outcomes logged | 10 | 5 | 5 | 20 |
| After 10 runs + outcomes | 20 | 10 | 10 | 40 |

The feedback loop is evident: each analysis deposits context into `market_insights` and `trade_history`, while each outcome reflection deposits lessons into `lessons_learned`. Subsequent analyses retrieve from all three stores, creating an increasingly rich context for decision-making.

### 4.4 Qualitative Analysis of Decision Quality

We evaluate decision quality through a case study approach, examining the AAPL analysis of 2026-02-07:

**Context-Aware Reasoning:** The Trader agent's HOLD decision at 0.75 confidence demonstrated integration of conflicting signals. Despite bullish momentum (+7.37% 30-day return) and positive news (Tim Cook announcement, analyst price targets at $300), the agent correctly weighed concerning fundamentals: a debt-to-equity ratio of 102.63 and a current ratio below 1.0 (0.974). This balanced weighing would not be possible in systems that lack the Research Manager's synthesis of opposing bull/bear arguments.

**Explainability:** The generated report provided full transparency into each agent's contribution, enabling a human analyst to trace the decision logic from raw data through analyst interpretations to the final recommendation. No output was truncated, ensuring complete auditability.

### 4.5 Outcome Learning Demonstration

To demonstrate the feedback loop, we simulate the following scenario:

1. **Session 1**: AgenticRAG issues BUY for NVDA at 0.80 confidence.
2. **Outcome**: After 5 trading days, NVDA returns −4.2%.
3. **Reflection**: The system generates a lesson: "❌ FAILED BUY for NVDA (−4.20% return). The analysis missed key factors — the RSI was in overbought territory and volume was declining. Be more cautious with similar setups."
4. **Session 2**: When NVDA is analyzed again, the KB Pre-Query retrieves this lesson and injects it into all analyst prompts as historical context.
5. **Result**: Analysts now explicitly reference the prior failure, leading to a more cautious assessment.

This adaptive behavior is the fundamental differentiator from static multi-agent systems.

### 4.6 Comparison with Baseline Architectures

Table 3 provides a qualitative comparison between AgenticRAG and relevant baseline architectures:

**Table 3.** Comparison of AgenticRAG with baseline architectures.

| Feature | Passive RAG | TradingAgents [4] | AgenticRAG (Ours) |
|---------|------------|-------------------|-------------------|
...
AgenticRAG makes three primary contributions to the field of Agentic RAG in finance:
...
Within the taxonomy proposed by Singh et al. [5], AgenticRAG implements elements from multiple categories:
...
- **Outcome Latency**: Lessons are generated only when actual returns are known (typically T+5 to T+30 days), creating a delay in the feedback loop. Future work could integrate real-time paper trading APIs for automated outcome capture.
...
This paper presented AgenticRAG, a multi-agent framework that advances the state of Agentic RAG by introducing an Active Knowledge Base Agent for financial investment decisions. The KB Agent's three-phase lifecycle — pre-query, post-learn, and outcome reflection — creates a self-improving system that accumulates institutional memory through a closed feedback loop, without requiring LLM retraining. The framework orchestrates 12 specialized agents through a LangGraph pipeline, grounded by three ChromaDB knowledge stores that separate market insights, trade history, and lessons learned.

Our experimental analysis demonstrates that the system produces contextually informed, fully explainable investment recommendations. The outcome learning mechanism enables the system to reference past successes and failures in future analyses, a capability absent from both passive RAG implementations and existing multi-agent trading frameworks. AgenticRAG represents a step toward truly adaptive AI systems in finance — systems that not only analyze markets but learn from their own experience.
